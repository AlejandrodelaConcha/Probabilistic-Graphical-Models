...
class HMM_LOG:
    ...
    def __init__(self, k, obs, init_mus, init_sigmas):
        ...
        Attris:
            ...
            log_p0 (np.array): the initial distribution of z0 on log scale
            log_A  (np.array): the transition on log scale
    ...
    def train(self, max_iter = 20, tol = 1e-6):
        ...
        for i in range(max_iter):
            ...
            self.loss.append(self.compute_log_likelihood())
    ...
    def compute_log_likelihood(self):
        """
        Calculate the likelihood function by the sequence of more likely states
        """
        
        #l = self.log_z0[self.preds[0]] + self.log_F[np.arange(self.n), self.preds].sum() + (self.log_A[self.preds[1:], self.preds[:-1]]).sum()
        log_l = self.log_alpha[self.n - 1] + self.log_beta[self.n - 1]
        lmax = log_l.max(0)

        l = lmax + logsumexp(log_l - lmax)
